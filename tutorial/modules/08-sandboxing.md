...
```mermaid
flowchart LR
    L1["📄 <b>L1</b><br/>纯文本 Agent<br/><i>不执行任何东西</i>"]
    L2["🚦 <b>L2</b><br/>Allowlist<br/><i>命令白名单</i>"]
    L3["📁 <b>L3</b><br/>Working dir 限制<br/><i>chroot / cwd</i>"]
    L4["⚙️ <b>L4</b><br/>Process 隔离<br/><i>ulimit / subprocess</i>"]
    L5["📦 <b>L5</b><br/>Container 隔离<br/><i>Docker / Colima</i>"]
    L6["🖥️ <b>L6</b><br/>VM 隔离<br/><i>Firecracker</i>"]
    L7["🚫 <b>L7</b><br/>离线沙箱<br/><i>airgap, no net</i>"]
    L1 --> L2 --> L3 --> L4 --> L5 --> L6 --> L7
    Weak(["最弱<br/>开发简单"]) -.-> L1
    L7 -.-> Strong(["最强<br/>开发复杂"])
    ...
    classDef strong fill:#1c3a2a,stroke:#4dffaa,stroke-width:2px,color:#fff
...
```mermaid
flowchart LR
    subgraph Box["🔒 <b>沙箱内</b>"]
      DO["🚀 <b>DevOps Agent</b><br/>不能 pip / python"]
    end

    subgraph Out["🌐 <b>沙箱外 (你的 host)</b>"]
      DASH["📊 <b>Dashboard</b><br/>POST /smoke-test"]
      Steps["1️⃣ python3 -m venv .venv<br/>2️⃣ pip install -r requirements.txt<br/>3️⃣ nohup python app.py &<br/>4️⃣ curl http://127.0.0.1:&lt;port&gt;/<br/>5️⃣ kill 子进程<br/>6️⃣ 收集 EVIDENCE"]
      DASH --> Steps
    end

    DO ==POST==> DASH
    Steps ==EVIDENCE block==> DO

    classDef sand fill:#3a1a1a,stroke:#ff4757,stroke-width:2px,color:#fff
    classDef host fill:#1c3a2a,stroke:#4dffaa,stroke-width:2px,color:#fff
