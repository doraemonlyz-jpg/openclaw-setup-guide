...
```mermaid
flowchart LR
    L1["<b>L1</b><br/>📜 提示约束<br/><span style='font-size:10px'>~70% → 90%</span>"]
    L2["<b>L2</b><br/>🧩 结构化输出<br/><span style='font-size:10px'>JSON Schema</span>"]
    L3["<b>L3</b><br/>🔍 Trust-but-verify<br/><span style='font-size:10px'>PM read 文件</span>"]
    L4["<b>L4</b><br/>🚪 Structural Gate<br/><span style='font-size:10px'>exit / HTTP code</span>"]
    L5["<b>L5</b><br/>🐕 Watchdog<br/><span style='font-size:10px'>独立兜底进程</span>"]

    L1 -->|加强| L2 -->|加强| L3 -->|加强| L4 -->|加强| L5

    Weak(["最弱"]) -.-> L1
    L5 -.-> Strong(["最强 / 99.9%"])

    classDef weak fill:#3a1a1a,stroke:#ff4757,stroke-width:2px,color:#fff
    classDef mid  fill:#2a1f4d,stroke:#b84dff,stroke-width:2px,color:#fff
    classDef strong fill:#1c3a2a,stroke:#4dffaa,stroke-width:3px,color:#fff
    classDef anchor fill:none,stroke:none,color:#94a3c4

    class L1,L2 weak
    class L3 mid
    class L4,L5 strong
    class Weak,Strong anchor
```
...
```mermaid
flowchart TB
    Worker["🤖 worker 声称<br/>'我做完了'"]
    L1{"L1<br/>persona 写 ⚠️ CRITICAL<br/>强调必须用工具"}
    L2{"L2<br/>tool schema 严格<br/>参数详细"}
    L3{"L3<br/>PM trust-but-verify<br/>read 验证文件"}
    L4{"L4<br/>QA gate (exit code)<br/>DevOps gate (HTTP code)"}
    L5{"L5<br/>Watchdog 兜底<br/>STATUS.json"}
    Truth(["✅ 老板看到的<br/><b>真实状态</b>"])

    Worker --> L1 --> L2 --> L3 --> L4 --> L5 --> Truth
    ...
    classDef wkr fill:#3a1a1a,stroke:#ff4757,stroke-width:2px,color:#fff
    classDef gate fill:#2a1f4d,stroke:#b84dff,stroke-width:2px,color:#fff
    classDef truth fill:#1c3a2a,stroke:#4dffaa,stroke-width:3px,color:#fff
    classDef lie fill:none,stroke:#ff4757,stroke-width:1px,stroke-dasharray:3 3,color:#ff4757

    class Worker wkr
    class L1,L2,L3,L4,L5 gate
    class Truth truth
    class Lie1,Lie2,Lie3 lie
```
